...
if (isset($_GET['action']) && $_GET['action'] == 'supprimer') {
    ...
    header('Location: affectations.php?message=' . urlencode('✅ Affectation supprimée.') . '&type=success');
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($check->num_rows == 0) {
        ...
        header('Location: affectations.php?message=' . urlencode('✅ Affectation enregistrée.') . '&type=success');
    } else {
        header('Location: affectations.php?message=' . urlencode('❌ Erreur : Cet employé ou ce véhicule est déjà affecté.') . '&type=error');
    }
    exit();
}
