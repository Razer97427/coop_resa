...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    // Redirection propre en gardant les filtres
...
<table>
    ...
    <?php if ($list->num_rows > 0): ?>
        ...
        <tr class="<?php echo $row['actif'] ? '' : 'archived'; ?>">
			...
			<td style="text-align: center; font-weight: bold; color: <?php echo ($row['CT'] > date('Y-m-d')) ? 'green' : 'red'; ?>;" class="CT" data-label="CT">
				<?php echo htmlspecialchars($row['CT']); ?>
			</td>
            ...
            <td data-label="Actions">
                ...
                <?php if ($row['actif']): ?>
                    <a href="vehicules.php?<?php echo http_build_query(array_merge($url_params, ['action' => 'toggle_type'])); ?>" 
                       ...
                       title="Passer en <?php echo $row['est_communal'] ? 'Attitré' : 'Communal'; ?>">
                       🔁 Type
                    </a>
                    
                    <a href="vehicules.php?<?php echo http_build_query(array_merge($url_params, ['action' => 'desactiver'])); ?>" class="action-btn cancel-btn">Désactiver</a>
                <?php else: ?>
                    <a href="vehicules.php?<?php echo http_build_query(array_merge($url_params, ['action' => 'reactiver'])); ?>" class="action-btn charge-btn">Réactiver</a>
                    <a href="vehicules.php?<?php echo http_build_query(array_merge($url_params, ['action' => 'supprimer'])); ?>" class="action-btn cancel-btn" onclick="return confirm('Supprimer définitivement ?')">Supprimer</a>
                <?php endif; ?>
            </td>
        </tr>
        <?php endwhile; ?>
